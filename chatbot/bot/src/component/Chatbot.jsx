...
import assistantImage from './assistant.png'; // Import the images
...
const sanitizeHTML = (html) => {
  // Allow only specific HTML tags (e.g., <a> tags with href and target attributes)
  const allowedTags = /<a\s+href="[^"]*"\s*target="_blank"[^>]*>.*?<\/a>/gi;
  const allowedAttributes = /href="[^"]*"|target="_blank"/gi;

  // Remove any HTML that doesn't match the allowed pattern
  let sanitized = html.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, ''); // Remove <script> tags
  sanitized = sanitized.replace(/on\w+="[^"]*"/gi, ''); // Remove event handlers (e.g., onclick, onmouseover)

  // Keep only allowed <a> tags and their content
  ...
  for (let i = elements.length - 1; i >= 0; i--) {
    ...
    if (element.tagName.toLowerCase() !== 'a') {
      ...
      // Ensure the <a> tag only has allowed attributes
